...
async def test_run_pipeline_task_cleans_partial_outputs_on_failure(db_session_factory, tmp_path, monkeypatch):
    ...
    # error_msg is sanitized for clients and must NOT leak raw exception strings/stack/paths.
    assert task.error_msg == "生成失败，请稍后重试。"
    assert "image service failed" not in (task.error_msg or "")
...
@pytest.mark.asyncio
async def test_run_pipeline_task_error_msg_sanitizes_internal_exception(db_session_factory, tmp_path, monkeypatch):
    """task.error_msg must not leak raw exception strings / internal paths / stack info."""
    leaked_secret = "/backend/app/internal/path/stacktrace.py::LeakyClass.method"
    leaky_message = f"Traceback (most recent call last): ... {leaked_secret} conn refused"
    ...
    async with db_session_factory() as db:
        user = User(email="pipeline-sanitize@example.com", password_hash="hash")
        db.add(user)
        await db.flush()
        project = Project(
            user_id=user.id,
            title="Pipeline Sanitize",
            source_text="足够长的文段用于验证错误信息脱敏。",
            style_writing="modern",
            style_visual="ink_wash",
            style_audio="ancient_male",
            status="running",
    ...
    assert task.status == "failed"
    assert task.error_msg == "生成失败，请稍后重试。"
    assert leaked_secret not in (task.error_msg or "")
    assert "Traceback" not in (task.error_msg or "")
